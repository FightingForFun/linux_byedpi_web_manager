...
final class LinuxProcessManager {
    ...
    public function findProcess(int $port, string $realFilePath): array {
        ...
        $quotedPort = "--port $port";
        if ($exePath === $realFilePath && strpos($cmdline, $quotedPort) !== false) {
            return [
                'существует' => true,
                'pid' => (int)$pid,
    ...
    public function startProcess(string $realFilePath, string $ipForRun, int $port, string $args): array {
        ...
        if (!empty($ipForRun)) {
            $commandParts[] = '--ip';
            $commandParts[] = escapeshellarg($ipForRun);
        }
        ...
        $command = implode(' ', $commandParts) . ' > /dev/null 2>&1 & echo $!';
    ...
    public function killProcess(int $pid): array {
        ...
        if (!empty($result)) {
            return ['ошибка' => "Ошибка завершения: $result"];
        }
...
try {
    ...
    $ipForRun = $validation['ip_для_запуска'];
    
    $args = $arguments;
    if ($hostsFileName !== null) {
        $args = "--hosts " . escapeshellarg($hostsFileName) . " " . $arguments;
    }
    ...
    switch ($action) {
        ...
        case 'проверить_и_запустить':
            ...
            $startResult = $manager->startProcess(
                $realFilePath,
                $ipForRun,
                $port,
                $args
